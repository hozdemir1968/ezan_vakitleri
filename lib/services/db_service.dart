...
class DBService {
  ...
  Future<void> createTables(Database database) async {
    ...
    await database.execute("""CREATE TABLE IF NOT EXISTS notificationtable (
          id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
          setted INTEGER,
  ...
  // NOTIFICATION
  Future<int> insertNotification(NotificationM model) async {
    try {
      final Database db = await initializeDatabase();
      int result = await db.insert(
        'notificationtable',
  ...
  Future<List<NotificationM>> getNotifications() async {
    try {
      final Database db = await initializeDatabase();
      final List<Map<String, dynamic>> queryResult = await db.query('notificationtable');
      return queryResult.map((e) => NotificationM.fromMap(e)).toList();
    } catch (e) {
      throw ('Bir sorun oluştu $e');
    }
  }
